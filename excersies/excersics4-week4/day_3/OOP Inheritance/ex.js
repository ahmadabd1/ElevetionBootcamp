...
class Teacher extends Person {
    ...
    giveGrade(student, coursname, grade) {
        student.receiveGrade(coursname, grade)
    }
    addCourse(course) {
...
class Principal extends Person {
    ...
    expelStudent(student){
        // this.students.filter(sn =>sn===student.name)
        // this.students.forEach(p=>p.name!==student.name)
        this.students = this.students.filter(user => user.name !== student.name );
...
// const t1 = new Teacher("Cassandra", 2002, 40000)
// const p = new Principal("ahmad",2017)
// p.hireTeacher(t1)
// // console.log(p)
